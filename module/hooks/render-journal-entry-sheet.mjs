...
export default function (application, element, context, options) {
  ...
  if (application.document.getFlag("Pendragon", "css-adventure-entry") ?? false) {
    ...
    if (
      (application.document.getFlag("Pendragon", "fixed-adventure-heading") ?? false) &&
      !element.classList.contains("fixed-adventure-heading")
    ) {
      ...
      if (typeof application.document.pages?.contents?.[0]?.id === "string") {
        const subheading =
          application.document.pages.get(application.document.pages.contents[0].id)?.flags?.Pendragon?.[
            "fixed-adventure-subheading"
          ] ?? "";
        if (subheading !== "") {
          ...
          div.innerHTML =
            '<div class="bookmark' +
            (short ? " short" : "") +
            '"><img src="systems/Pendragon/assets' +
            (short ? "bookmarks.webp" : "bookmark.webp") +
            '"></div><div class="adventure-heading"><div class="heading">' +
            application.title +
            "</div>" +
            (short ? "" : '<div class="subheading">' + subheading + "</div>") +
            "</div>";
