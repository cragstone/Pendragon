export class PENActorData extends foundry.abstract.TypeDataModel {
  ...
  _calcDV() {
    ...
    for (let [key, dv] of Object.entries(this.dv)) {
    ...
    if (this.dv.motte1.value > 0 && this.dv.motte1.singlemotte) {
      this.dv.motte1.value = this.dv.motte1.value + this.dv.stronghold1.value;
    }
    if (this.dv.motte2.value > 0 && this.dv.motte2.singlemotte) {
      this.dv.motte2.value = this.dv.motte2.value + this.dv.stronghold2.value;
    }
    if (this.dv.motte3.value > 0 && this.dv.motte3.singlemotte) {
      this.dv.motte3.value = this.dv.motte3.value + this.dv.stronghold3.value;
    }
  ...
  _calcFolkCost() {
    ...
    for (let i of this.parent.items) {
      if (i.type === "skill") {
        ...
        i.system.total = Number(i.system.value) + Number(i.system.religious) + Number(i.system.winter);
      }
    }
    //Calculate Skill/Trait Cost
    ...
    for (let backNPC of backList) {
      //Skill Cost is based on the the sum of skills associated with the background npc
      ...
      let skillCost = 0;
      for (let skill of theseSkills) {
        skillCost = skillCost + Math.max(0, skill.total - 15) + Math.max(0, skill.total - 19);
      }
      backNPC.system.skillCost.libra = skillCost;
      let totalCost =
    ...
    let folkCost = 0;
    let manorFolk = this.parent.items.filter((i) => i.type === "background");
    ...
    if (this.parent.type === "barony") {
      this.folkCost.libra = Math.round(folkCost / 240);
      this.folkCost.denarii = 0;
    } else if (this.parent.type === "manor") {
      this.folkCost.libra = Math.floor(folkCost / 240);
      this.folkCost.denarii = folkCost % 240;
    }
    return;
  }
  ...
  _getStewardship() {
    ...
    let stewardship = 0;
    if (this.enfeoffed.npc) {
      let tempActor = fromUuidSync(this.enfeoffed.npc);
      if (tempActor) {
    ...
    } else if (this.enfeoffed.background != "none") {
      let backgroundNPC = this.parent.items.get(this.enfeoffed.background).uuid;
      if (backgroundNPC) {
    ...
    this.enfeoffed.stewardship = stewardship;
  }
}
