...
export class PENCharCreate {
  ...
  static async startCreate(actor, step, undo) {
    ...
    if (actor.system.create.complete || actor.type != "character") {
      return;
    }
  ...
  static async undoCreate(actor, step) {
    ...
    let confirmation = await PENUtilities.confirmation(
      game.i18n.localize("PEN.step." + step) + " " + game.i18n.localize("PEN.undoStep"),
    );
    if (!confirmation) {
      return;
    }
  ...
  static async step2(actor) {
    ...
    for (let cCount = 1; cCount <= maxHeroic; cCount++) {
      ...
      await PENCharCreate.createHistory(actor, title, year, 0, "inherited", description);
    }
    ...
    if (actor.system.classID === "") {
      ...
      if (currentClass) {
        if (itemData[0].flags?.Pendragon?.pidFlag?.id === "i.class.mercenary-knight") {
          let oldPassion = (
            await actor.items.filter((i) => ["i.passion.homage-lord"].includes(i.flags?.Pendragon?.pidFlag?.id))
          )[0];
          let newPassion = (
            await actor.items.filter((i) => ["i.passion.fealty-lord"].includes(i.flags?.Pendragon?.pidFlag?.id))
          )[0];
  ...
  static async step15(actor) {
    let data = {
      msg: game.i18n.localize("PEN.beKnighted"),
  ...
  static async selectActorItem(actor, type, title) {
    ...
    let newList = await actor.items
      ...
      .map((itm) => {
        return { name: itm.name, pid: itm.flags.Pendragon.pidFlag.id, system: { value: itm.system.total } };
      });
  ...
  static async validate() {
    ...
    let list = await game.system.api.pid.fromPIDRegexBest({
      pidRegExp: new RegExp("^i." + PENUtilities.quoteRegExp("culture") + ".+$"),
      type: "i",
    });
    if (list.length < 1) {
      ui.notifications.error(game.i18n.localize("PEN.noCultures"));
    ...
    list = await game.system.api.pid.fromPIDRegexBest({
      pidRegExp: new RegExp("^i." + PENUtilities.quoteRegExp("class") + ".+$"),
      type: "i",
    });
    if (list.length < 1) {
    ...
    list = await game.system.api.pid.fromPIDRegexBest({
      pidRegExp: new RegExp("^i." + PENUtilities.quoteRegExp("homeland") + ".+$"),
      type: "i",
    });
    if (list.length < 1) {
      ui.notifications.error(game.i18n.localize("PEN.noHomelands"));
    ...
    list = await game.system.api.pid.fromPIDRegexBest({
      pidRegExp: new RegExp("^i." + PENUtilities.quoteRegExp("religion") + ".+$"),
      type: "i",
    });
    if (list.length < 1) {
      ui.notifications.error(game.i18n.localize("PEN.noReligions"));
  ...
  static async addCulture(actor, culture) {
    ...
    for (let item of actor.items) {
      ...
      await Item.updateDocuments(changes, { parent: actor });
      await actor.update({
        "system.stats.siz.culture": Number(culture.system.stats.siz.bonus),
  ...
  static async resetBaseSkillScores(actor) {
    let skills = actor.items
      .filter((itm) => itm.type === "skill")
      .map((itm) => {
        return { _id: itm.id, "system.value": 0 };
      });
    await Item.updateDocuments(skills, { parent: actor });
    return;
  }
